...
/**
 * ★★★ 仮のログインユーザーID ★★★
 * AddFriend3D.tsx と共通の値を使用してください。
...
    
    if (!CURRENT_USER_ID) { 
      console.error("CURRENT_USER_ID が設定されていません。");
      setIsLoading(false);
      return;
    }
...
      `)
      .or(`user_id_1.eq.${CURRENT_USER_ID},user_id_2.eq.${CURRENT_USER_ID}`);
...
          const counterParty = fs.user_id_1 === CURRENT_USER_ID ? fs.user2 : fs.user1;
...
          } 
          else if (fs.status === 'PENDING' && fs.requester_id !== CURRENT_USER_ID) {
              pending.push({ 
