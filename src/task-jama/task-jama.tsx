...
// 文字化け関数のインポート
import { createMojibakeText, MojibakeResult } from './components/todo-mojibake';
...
const TaskJama: React.FC<TaskJamaProps> = ({ userId, onBackToHome }) => {
...
    }
    ...
    try {
      // 0. ターゲットのToDoの現在のタイトルを取得
      const { data: todoData, error: fetchError } = await supabase
        .from('todos')
        .select('title, original_title, disruption_count') // original_title も選択（あれば）
        .eq('todo_id', targetTodoId)
        .single();

      if (fetchError || !todoData) {
        throw new Error(`ターゲットToDoの取得エラー: ${fetchError?.message}`);
      }

      const trueOriginalTitle = todoData.original_title || todoData.title; 
      const currentDisruptCount = todoData.disruption_count || 0; 
      const newDisruptCount = currentDisruptCount + 1;

      const mojibakeResult: MojibakeResult = createMojibakeText(
          trueOriginalTitle, 
          todoData.title, // 現在の表示タイトルを渡す
          newDisruptCount
      );

      const isFullyDisrupted = newDisruptCount >= 3;
...
      // 2.5. todos テーブルの該当ToDoを妨害済みとしてマーク 
      const { error: todoUpdateError } = await supabase
        .from('todos')
        .update({ 
            is_disguised: true, // 妨害済みフラグを立てる
            title: mojibakeResult.disguisedText,
            original_title: trueOriginalTitle,
            disruption_count: newDisruptCount,
            is_fully_disrupted: isFullyDisrupted,
            updated_at: new Date().toISOString()
        })
        .eq('todo_id', targetTodoId);

      if (todoUpdateError) {
        throw new Error(`ToDo更新エラー: ${todoUpdateError.message}`);
      }

      if (isFullyDisrupted) {
          alert('このタスクは完全に文字化けされたぞ！');
      } else {
          alert('妨害に成功しました！'); 
      }
...
                />
                ...
                {/* ホームに戻るボタン */}
                            ...
                            >
                                ...
                                <Text position={[0, 0, 0.26]} fontSize={0.15} color="white">
                                    ホーム
                                </Text>
                            </mesh>
...
                >
                  <boxGeometry args={[1, 0.4, 0.1]} />
                  <meshStandardMaterial color={pendingRequests.length > 0 ? "#ffc107" : "#ffc107"} /> 
...
                  onJamaComplete={(todoId) => handleJamaComplete(todoId)}
                />
              )}
              
